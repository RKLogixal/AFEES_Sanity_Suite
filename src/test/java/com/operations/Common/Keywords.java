...
public class Keywords {
	...
	public void CLICK_JAVASCRIPT_ELEMENT(WebDriver driver,Properties p,String objectName,String objectType,ExtentTest test,Logger log) throws Exception {
	...
	public void VERIFYTEXT_LIST(WebDriver driver,Properties p,String objectName,String objectType,String value,ExtentTest test,Logger log) throws Exception {

		List <WebElement> VList =driver.findElements(this.getObject(p, objectName, objectType));
		int VListSize=VList.size();

		for (int vl=0;vl<VListSize;vl++) {

			String VString = VList.get(vl).getText();

			if (VString.equalsIgnoreCase(value)) {

				log.info(value + ": found from expected list....");

				break;
			}
			else if (vl==(VListSize-1)) {
				Assert.fail("Expected Text i.e." + value + " Not found from the list...");
			}
		}
	...
	public void CLICK_LIST(WebDriver driver,Properties p,String objectName,String objectType,String value,ExtentTest test,Logger log,String Domain) throws Exception {

		String[] Conditions =value.split(",");

		String cpart1 = Conditions[0]; 
		String cpart2 = Conditions[1]; 

		List <WebElement> CList =driver.findElements(this.getObject(p, objectName, objectType));
		int CListSize=CList.size();

		for (int cl=0;cl<CListSize;cl++) {

			String CString = CList.get(cl).getAttribute("href");
			//System.out.println(CString);


			if (CString.contains(cpart1) && CString.contains(cpart2) ) {

				String[] Remove_domain =CString.split(Domain);
				String part1 = Remove_domain[0]; 
				String part2 = Remove_domain[1]; 
				String xpath = "//a[@href='"+part2+"']";
	...
	public void TEMP(WebDriver driver,Properties p,String objectName,String objectType,String value,ExtentTest test,Logger log,String Domain) throws Exception {
		...
		List <WebElement> CList =driver.findElements(By.xpath("//div[@class='container-item-controls']//following-sibling::span[1]/a[1]"));
		int CListSize=CList.size();
		...
		for (int cl=0;cl<CListSize;cl++) {
			...
			String CString = CList.get(cl).getAttribute("href");
			System.out.println(CString);
			if (CString.contains("AUTO_TEST")) {
				String[] Remove_domain =CString.split("http://129.213.54.196:8002");
				String part1 = Remove_domain[0]; 
				String part2 = Remove_domain[1]; 
				String xpath = "/a[contains(@href'AUTO_TEST')]";
				String dxpath="//div[@class='container-item-controls' and //span[1][@class='edit-button']"+xpath+"]/parent::div/div/span[2]/a[@href]";
				System.out.println(dxpath);
				List <WebElement> MList =driver.findElements(By.xpath(dxpath));
				int MListSize=MList.size();
				System.out.println(MListSize);
	...
	public void CLEAR_ALL_ITEMS(WebDriver driver,Properties p,String objectName,String objectType,String value,ExtentTest test,Logger log) throws Exception {
		//WebDriverWait myWaitVar = new WebDriverWait(driver,20);

		List <WebElement> t1 =driver.findElements(this.getObject(p, objectName, objectType));
		System.out.println("Total items found to delete:" + t1.size());
			...
			for(int i =0;i<=t1.size()-1;i++)
			{
				...
				if (Click.booleanValue()==true){
					...
					synchronized (LOCK) {
						...
						t2.get(0).click();
						t2.remove(0);
						driver.findElement(By.xpath("//*[contains(@id,'popover')]/div/div/div/div/a[contains(text(),'Yes')]")).click();
